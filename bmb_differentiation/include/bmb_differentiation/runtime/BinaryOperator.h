...
class BinaryOperator : public Expression {
 public:
  ...
  explicit BinaryOperator(ExprPtr first, ExprPtr second)
 ...
 protected:
  ...
  [[nodiscard]] virtual double call(const double& first,
                                    const double& second) const = 0;

  [[nodiscard]] virtual ExprPtr call(const ExprPtr& first,
                                     const ExprPtr& second) const = 0;
  ...
  // This function only guarantees the left distributive property. The right
  // distributive property is implied given distributivity and commutativity.
