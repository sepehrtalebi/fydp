...
ExprPtr Power::diff(const std::string& id) const {
  return pow(first, second) *
         (second->diff(id) * log(first) + first->diff(id) * (second / first));
}
...
std::string Power::type() const { return "pow"; }
...
ExprPtr sqrt(const ExprPtr& expr) { return pow(expr, Constant::make(1. / 2.)); }
