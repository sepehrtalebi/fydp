...
double Sin::call(const double& operand) const { return sin(operand); }

ExprPtr Sin::call(const ExprPtr& operand) const { return sin(operand); }

ExprPtr Sin::derivative(const ExprPtr& expr) const { return cos(expr); }
