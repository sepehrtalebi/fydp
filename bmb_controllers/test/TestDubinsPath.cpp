...
TEST(TestDubinsPath, testDubinsPath) {
    ...
    ASSERT_TRUE(path[0].is_turning && path[0].isRightTurn() &&
                !path[1].is_turning && path[2].is_turning && path[2].isRightTurn());
    ASSERT_TRUE(std::fabs((path.closestPoint(Vector2{4.5, 1}) - Vector2{4.5, 1}).magnitude()) < 0.001);

    std::ofstream out1(path + "dubins_path1.csv");
    ASSERT_TRUE(out1);
    path.toCSV(out1);
    out1.close();
    ...
    std::ofstream out2(path + "dubins_path2.csv");
    ASSERT_TRUE(out2);
    path.toCSV(out2);
    out2.close();
    ...
    path = DubinsPath<double>::create(start, goal, radius);
    std::ofstream out3(path + "dubins_path3.csv");
    ASSERT_TRUE(out3);
    path.toCSV(out3);
    out3.close();
}
