...
class Expression {
  // Note that we can't declare variables of type Expression since it is
  // abstract. Thus, we also can't have them as elements in Vector The
  // workaround is to use ExprPtr in its place, and define global arithmetic
  // operators to work with it
 public:
  ...
  [[nodiscard]] virtual ExprPtr subs(
      const std::unordered_map<std::string, ExprPtr>& subs) const = 0;
...
// This ensures all these operators can be accessed, even if only Expression.h
// is #included
