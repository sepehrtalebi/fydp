...
void LocalPathPlanner::referenceCommandCallBack(
    bmb_msgs::ReferenceCommand ref_msg) {
  this->ref_cmd = ref_msg;
  this->update_dubins = true;
}

bmb_msgs::StateCommand LocalPathPlanner::update(
    const bmb_msgs::AircraftState& state_msg) {
  ...
  double vertical_force =
      altitude_pid.update(state_msg.pose.position.z, ref_cmd.altitude);
