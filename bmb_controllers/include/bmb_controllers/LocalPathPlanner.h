...
class LocalPathPlanner {
  /**
      void spin() {
          ...
          typename PurePursuit<T>::Result result = pursuer.pursue(path,
  current_state); if (result.should_replan) { path =
  DubinsPath<T>::create(current_state, goal, 5); result = pursuer.pursue(path,
  current_state);
          }
          T ang_vel = result.angular_vel;
          T vertical_force = altitude_pid.update(0); // TODO: current height
          T horizontal_force = 0; // TODO: compute from ang_vel and forward
  velocity
