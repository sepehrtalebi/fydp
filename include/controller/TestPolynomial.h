...
class TestPolynomial {
    ...
    bool test1() {
        ...
        ASSERT_TRUE(poly.get_size() == 0);
        for (int i = 2; i < 5; i++)
            poly.push(double(i));
        for (int i = 0; i < 3; i++)
            ASSERT_TRUE(poly[2 - i] == double(i + 2));
        ASSERT_TRUE(poly.get_size() == 3);
        return true;
    }
    ...
    bool test2() {
        ...
        for (int i = 0; i < 3; i++)
            ASSERT_TRUE(poly[2 - i] == double(i + 2));
        poly *= poly;
        ASSERT_TRUE(poly.get_size() == 5);
        std::vector<double> test{4, 12, 25, 24, 16};
        for (int i = 0; i < 5; i++)
            ASSERT_TRUE(poly[i] == test[i]);
        poly *= 2;
        for (int i = 0; i < 5; i++)
            ASSERT_TRUE(poly[i] == 2 * test[i]);
        poly += Polynomial<double> {test};
        for (int i = 0; i < 5; i++)
            ASSERT_TRUE(poly[i] == 3 * test[i]);
        return true;
    }
